...
class CompletedBeforeFragment : BindingFragment<FragmentCompletedBeforeBinding>() {
    override fun onBindingCreated(savedInstanceState: Bundle?) {
        binding.weekBtn.onClick {
            FragmentRootActivity.start(requireContext(), CompletedFragment::class.java, arguments = Bundle().also { it.putBoolean("isWeek", true) })
        }

        binding.dayButton.onClick {
            FragmentRootActivity.start(requireContext(), CompletedFragment::class.java, arguments = Bundle().also { it.putBoolean("isWeek", false) })
        }
    }
