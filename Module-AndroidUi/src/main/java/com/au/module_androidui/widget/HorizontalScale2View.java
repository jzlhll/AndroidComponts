...
public class HorizontalScale2View extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!isMeasured) {
            ...
            minScaleLength = maxScaleLength * 2 / 3;
    ...
    private final int[] scaleWidthIndexJ = new int[5];

    private final List<Rect> cacheRectList = new ArrayList<>();
    private final Path mDaoTripPath = new Path();
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        //画刻度线
        for (int i = min; i <= max; i++) {
            var x = minX + (i - min) * scaleSpaceUnit;
            ...
            int w = rect.width();
            int h = rect.height();
            canvas.drawText(str, x - w / 2f - hfScaleWidthBig, ruleHeight - maxScaleLength - h - minScaleLength / 2f, paint);
            //画大刻度线
            paint.setStrokeWidth(SCALE_WIDTH_BIG);
            canvas.drawLine(x, ruleHeight - maxScaleLength, x, ruleHeight, paint);
            ...
            canvas.drawLine(x + hfScSpaceUnit, ruleHeight, x + hfScSpaceUnit, ruleHeight - midScaleLength, paint);
            //画小刻度线
            for (int j = 1; j < 5; j++) {
                var smallX = x + scaleWidthIndexJ[j];
        ...
        canvas.drawLine(halfWidth, 0, halfWidth, maxScaleLength, paint);
        //画倒三角形指针
        {
            Path path = mDaoTripPath;
            path.reset();
            path.moveTo(halfWidth - 10, 0);
            path.lineTo(halfWidth + 10, 0);
